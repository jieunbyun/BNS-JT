...
for measureInd = 0:nMeasurement
    ...
    corrosionRateDistributionType = 'Normal';
...
% Exact inference
load ex2_steelPipe_exactInference
lineWidth = 1.5; fontSizeLegend = 14; fontSizeTick = 13; fontSizeLabel = 16;
lineShape = {'+--' 'o--' '*--'};
exactResultColorSpec = [0.9290 0.6940 0.1250; 0.8500 0.3250 0.0980; 0 0.4470 0.7410];

for plotInd = 1:size( systemFailureProb, 2 )
    iTimePlot = 1:timeForPlot(plotInd);
    h{plotInd} = plot( iTimePlot, systemFailureProb( iTimePlot,plotInd ), lineShape{plotInd}, 'LineWidth', lineWidth, 'Color', exactResultColorSpec( plotInd,: ) );
end    
set(gca, 'YScale', 'log')
grid on
...
saveas(gcf,'figure/steelPipe_failureProbability_normal.emf')
saveas(gcf,'figure/steelPipe_failureProbability_normal.pdf')